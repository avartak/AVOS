...
uint32_t Process_Fork() {
	...
	forked_proc->memory_endpoint = parent_proc->memory_endpoint;
...
void Process_ChangeMemoryEndPoint(int32_t shift) {
    ...
    if (new_end == old_end) return;
        ...
        Paging_UnmapPages(proc, (void*)new_end, old_end - new_end);
        proc->memory_endpoint = new_end;
    }

    else {
        size_t size_alloc = Paging_MapPages(proc, (void*)old_end, new_end - old_end);
