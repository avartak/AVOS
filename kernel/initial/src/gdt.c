#include <kernel/initial/include/gdt.h>
...
void GDT_Initialize() {
	...
	GDT_SetupEntry(&(GDT_entries[1]), 0x00000000  , 0x000FFFFF   , GDT_KERN_CODE_SEG_ACCESS, GDT_32BIT_PMODE | GDT_4GB_SEGMENTSIZE);
	GDT_SetupEntry(&(GDT_entries[2]), 0x00000000  , 0x000FFFFF   , GDT_KERN_DATA_SEG_ACCESS, GDT_32BIT_PMODE | GDT_4GB_SEGMENTSIZE);
	GDT_SetupEntry(&(GDT_entries[3]), 0x00000000  , 0x000FFFFF   , GDT_USER_CODE_SEG_ACCESS, GDT_32BIT_PMODE | GDT_4GB_SEGMENTSIZE);
	GDT_SetupEntry(&(GDT_entries[4]), 0x00000000  , 0x000FFFFF   , GDT_USER_DATA_SEG_ACCESS, GDT_32BIT_PMODE | GDT_4GB_SEGMENTSIZE);
	GDT_SetupEntry(&(GDT_entries[5]), tss_seg_base, tss_seg_limit, GDT_TSS_SEG_TYPE_ACCESS , GDT_TSS_FLAGS                        );
