...
#define ICW1_NEED_ICW4         0x01    /* ICW4 (not) needed           -- we need it and so the bit needs to be set */
#define ICW1_SINGLE            0x02    /* Single (cascade) mode       -- we will initialize in cascade mode i.e. one master and one slave PIC, and so this bit needs to be 0 */
#define ICW1_CALL4             0x04    /* Call address interval 4 (8) -- not relevant for 8086 mode that we will be using, set to 0 */
#define ICW1_LEVEL             0x08    /* Level triggered (edge) mode -- edge triggered */
#define ICW1_INIT              0x10    /* Initialization - required!  -- this bit needs to be set to perform the initialization sequence */
 
#define ICW4_8086              0x01    /* 8086/88 (MCS-80/85) mode    -- this is the mode we are interested in, should be set */
#define ICW4_AUTO_EOI          0x02    /* Auto (normal) EOI           -- normal EOI will be dispatched by the interrupt handler, set to 0 */
#define ICW4_BUF_SLAVE         0x08    /* Buffered mode/slave         -- no, set to 0 */
#define ICW4_BUF_MASTER        0x0C    /* Buffered mode/master        -- no, set to 0 */
#define ICW4_SFNM              0x10    /* Special fully nested (not)  -- no, set to 0 */
...
#define PIC1_REMAP_START       0x020   /* Start interrupt vector for the remapped IRQs of the master PIC */
#define PIC2_REMAP_START       0x028   /* Start interrupt vector for the remapped IRQs of the  slave PIC */
